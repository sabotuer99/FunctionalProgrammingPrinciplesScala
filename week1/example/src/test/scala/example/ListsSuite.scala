 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a few huge numbers") {
    assert(sum(List(1 << 28,1 << 30, 1 << 29)) === (7 << 28))
  }
  ...
  test("max throws exception on empty list") {
  ...
  test("max negative numbers") {
    assert(max(List(-1, -2, Integer.MIN_VALUE)) === -1)
  }

  test("max single element") {
    assert(max(List(42)) === 42)
  }
